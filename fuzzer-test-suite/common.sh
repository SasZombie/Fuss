...
FUZZ_CXXFLAGS="-O2 -fno-omit-frame-pointer -g3 -fsanitize=address -fsanitize-address-use-after-scope -fsanitize-coverage=trace-pc-guard,trace-cmp,trace-gep,trace-div"
...
#Only tested env for my project
if [[ $FUZZING_ENGINE == "fsanitize_fuzzer" ]]; then
  CLANG_PROFILING_FLAGS=""
  
  if [ "$PROFILE" = true ]; then

    mkdir -p "$(pwd)/profiles"
    
    CLANG_PROFILING_FLAGS="-fprofile-generate=$(pwd)/profiles"
    FSANITIZE_FUZZER_FLAGS="-O2 -w -fno-omit-frame-pointer -g3 -fsanitize=address,fuzzer-no-link -fsanitize-address-use-after-scope"
    ...
    FSANITIZE_FUZZER_FLAGS="-O2 -w \
      ...
      -fsanitize-address-use-after-scope -fno-builtin"
  fi

  export CFLAGS="${FSANITIZE_FUZZER_FLAGS} ${CLANG_PROFILING_FLAGS}"
  export CXXFLAGS="${FSANITIZE_FUZZER_FLAGS} ${CLANG_PROFILING_FLAGS}"
